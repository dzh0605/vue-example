<template>
  <div class="container">
    <Breadcrumb :items="['按钮', '流动渐变按钮']" />

    <a-card class="text">
      通过左侧表单控制中间按钮普通样式，右侧表单控制按钮渐变样式，可以实现按钮的流动渐变效果。
    </a-card>
...
const buttonInfo = reactive({
  ...
  // 渐变设置
  gradientDirection: 0,
  gradientColor: ["#BEDAFF", "#DDBEF6", "#B7F4EC"],
})
...
const setStyle = () => {
  ...
  // // 渐变背景
  buttonSign.value.style.background = `linear-gradient(90deg
  , ${buttonInfo.gradientColor.join(",")}
  , ${buttonInfo.gradientColor.join(",")}
  , ${buttonInfo.gradientColor.join(",")}
  , ${buttonInfo.gradientColor.join(",")}
  , ${buttonInfo.gradientColor.join(",")})`

  buttonSign.value.style.backgroundSize = "500% 500%"
  buttonSign.value.style.backgroundPosition = "40% 40%"
  buttonSign.value.style.animation = "gradient 1s linear infinite"

  // 渐变动画
  const gradient = buttonSign.value.style.background
  ...
  const keyframes = `
    ...
    to {
      background-position: 13.5% 40%;
    }
  `
  const style = document.createElement('style')
  style.textContent = `@keyframes gradient {${keyframes}}`
