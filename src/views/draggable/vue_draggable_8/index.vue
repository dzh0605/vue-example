<template>
  <div class="container">
    <Breadcrumb :items="['元素拖拽', '拖拽数量限制（vue.draggable.next）']" />

    <a-card class="text">
      拖拽进出限制，只可从左侧列表拖入右侧列表，右侧列表最多只能有5个元素。
    </a-card>
    ...
    <a-card class="general-card">
      <a-row>
        ...
        <a-col :span="6" class="box-2">
          <draggable :list="list_2" animation="300" :group="{ name: 'people', pull: false, put: putFn }"
            :force-fallback="true" :ghost-class="ghostClass" :chosen-class="chosenClass" :drag-class="dragClass">
...
// 限制右侧列表最多只能有5个元素
const putFn = () => {
  return list_2.value.length < 5;
}

// 开始拖拽时，判断右侧列表是否已满
const start = () => {
  if (list_2.value.length >= 5) {
    Message.warning("右侧列表最多只能有5个元素");
  }
}
</script>
